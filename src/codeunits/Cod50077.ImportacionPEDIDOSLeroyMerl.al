...
Codeunit 50077 "Importacion PEDIDOS Leroy Merl"
{
    ...
    begin
        ...
        ADAIA.Reset();
        ADAIA.SetRange(texto, 'IMPORTACION LEROYMERLIN CU-50077');
        IF ADAIA.FindSet() THEN begin
            nomdir := ADAIA.Ruta;
            RUTACOPIA := ADAIA.Ruta + 'copia/';
        end;
        Commit;
        tipo := Tipo::LEROYMERLIN;


        FicherosHagen.LeerArchivosCarpeta(nomdir, RUTACOPIA, Tipo)
        /////                               Xmlport.Import(Xmlport::"Importacion PEDIDOS Leroy Merl", varInputStream);
