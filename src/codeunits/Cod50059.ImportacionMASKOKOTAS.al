...
Codeunit 50059 "Importacion MASKOKOTAS"
{

    trigger OnRun()
    var
        FicherosHagen: Codeunit FicherosHagen;
        Tipo: option MASKOKOTAS;

    begin


        ADAIA.Reset();
        ADAIA.SetRange(texto, 'IMPORTACION MASKOKOTAS-CU-50059');
        IF ADAIA.FindSet() THEN begin
            nomdir := ADAIA.Ruta;
            RUTACOPIA := ADAIA.Ruta + 'copia/';
        end;
        Commit;
        tipo := Tipo::MASKOKOTAS;


        FicherosHagen.LeerArchivosCarpeta(nomdir, RUTACOPIA, Tipo)

        ///Xmlport.Import(Xmlport::"Importacion PEDIDOS MASKO-new", varInputStream);
