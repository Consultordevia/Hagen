...
Codeunit 50078 "Importacion CATIT nuevo"
{
    ...
    begin
        ...
        ADAIA.Reset();
        ADAIA.SetRange(texto, 'IMPORTACION CATITNUEVO-CU-50078');
        IF ADAIA.FindSet() THEN begin
            nomdir := ADAIA.Ruta;
            RUTACOPIA := ADAIA.Ruta + 'copia/';
        end;
        Commit;
        tipo := Tipo::CATITNUEVO;


        FicherosHagen.LeerArchivosCarpeta(nomdir, RUTACOPIA, Tipo)

        /////                                   Xmlport.Import(Xmlport::"Importacion PEDIDOS catitt new", varInputStream);
