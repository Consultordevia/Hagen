...
Report 50101 "Importa Grupos Dtos"
{
    ...
    begin
        ...
        if (prod='') then begin
             ...
             RecLT."Line No.":=LIN;
             RecLT."No.":=codprod;
             RecLT."Language Code":='PTG';
             RecLT.Text:=CopyStr(texto,1,44);
             RecLT."Text No.":=1;
             RecLT.Insert(true);
        end;
        if (prod<>'') then begin
             if not RecCT.Get(4,prod,'PTG',1) then begin
                  RecCT."Table Name":=4;
                  RecCT."No.":=codprod;
                  RecCT."Text No.":=1;
                  RecCT."Language Code":='PTG';
             ...
             if RecTP.Get(prod,'','PTG') then begin
                  RecTP.Description:=CopyStr(texto,1,44);
                  RecTP.Modify(true);
             end;
             if not RecTP.Get(prod,'','PTG') then begin
                  RecTP."Item No.":=prod;
                  RecTP."Variant Code":='';
