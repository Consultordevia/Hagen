...
{	
	...
	{
		type(ForNav.Report_7_3_0_2521; ForNavReport50017_v7_3_0_2521){}   
	}
	assembly("mscorlib")
	{
		Version='4.0.0.0';
		type("System.IO.Stream"; SystemIOStream50017){}   
		type("System.IO.Path"; System_IO_Path50017) {}
	}
} // Reports ForNAV Autogenerated code - do not delete or modify -->

Report 50017 "Factura de venta"
{
	Caption = 'Factura de venta';
	Permissions = TableData "Sales Shipment Buffer"=rimd;
	RDLCLayout = './Layouts/Facturadeventa.rdlc'; DefaultLayout = RDLC;
	...
	dataset
	{
		dataitem("Sales Invoice Header";"Sales Invoice Header")
		{
			...
			column(textodiaspago; textodiaspago)
			{
			}
			...
			dataitem(CopyLoop;Integer)
			{
				...
				dataitem(PageLoop;Integer)
				{
					...
					dataitem("Sales Invoice Line";"Sales Invoice Line")
					{
						...
						DataItemLinkReference = SalesInvoiceHeader;
						DataItemTableView = sorting("Document No.","Shipment No.",Ean);
						column(ReportForNavId_60; 60) {} // Autogenerated by ForNav - Do not delete
						...
						column(LineAmount6; "Sales Invoice Line"."Line Amount")
						{
						...
						column(TipoLinea; "Sales Invoice Line".Type)
						{
						}
						...
						trigger OnAfterGetRecord();
						begin
							if ("Sales Invoice Line".Quantity=0) and ("Sales Invoice Line".Type<>0) then begin
								 CurrReport.Skip;
							end;
							...
							PV:=ROUND(precioreco/(1+("Sales Invoice Line"."VAT %"/100)));
							PV2:=PV-"Sales Invoice Line"."Precio base";
							if PV<>0 then begin
							BF:=ROUND(PV2/PV,0.01)*100;
							end;
							INFOLINEA:='PVP:'+Format(precioreco)+' PV:'+Format(PV)+' BENEF:'+Format(BF)+'%';
							end;
							dto:='';
							...
							if "Sales Invoice Line"."Precio final"=0 then begin
								 if "Sales Invoice Line".Quantity<>0 then begin
									  "Precio final":=ROUND("Line Amount"/Quantity,0.001);
								 end;
							end;
							codean:=Ean;
			...
			begin
				...
				if "Sales Invoice Header"."Pay-at Code"<>'' then begin
					 ///// falta FormatAddr.DirePago(CustAddr,"Sales Invoice Header");
				end;
				if not Cust.Get("Bill-to Customer No.") then
				  Clear(Cust);
				CustAddrdf[1]:="Sales Invoice Header"."Sell-to Customer Name";
				CustAddrdf[2]:="Sales Invoice Header"."Sell-to Customer Name 2";
				CustAddrdf[3]:="Sales Invoice Header"."Sell-to Address";
				CustAddrdf[4]:="Sales Invoice Header"."Sell-to Address 2";
				CustAddrdf[5]:="Sales Invoice Header"."Sell-to Post Code"+' '+"Sales Invoice Header"."Sell-to City";
				CustAddrdf[6]:="Sales Invoice Header"."Sell-to County";
				CustAddrdf[7]:="Sales Invoice Header"."Sell-to Country/Region Code";
				CustAddrdf[8]:='';
				if CustAddrdf[2]='' then begin
					 CustAddrdf[2]:=CustAddrdf[3];
					 CustAddrdf[3]:=CustAddrdf[4];
					 CustAddrdf[4]:=CustAddrdf[5];
					 CustAddrdf[5]:=CustAddrdf[6];
					 CustAddrdf[6]:=CustAddrdf[7];
					 CustAddrdf[7]:='';
					 if CustAddrdf[3]='' then begin
						  CustAddrdf[3]:=CustAddrdf[4];
						  CustAddrdf[4]:=CustAddrdf[5];
						  CustAddrdf[5]:=CustAddrdf[6];
						  CustAddrdf[6]:=CustAddrdf[7];
				...
				RecEAD.Reset;
				RecEAD.SetRange(RecEAD."Customer No.","Sales Invoice Header"."Sell-to Customer No.");
				...
				if UserId='20' then begin
					 RecFP.Get("Sales Invoice Header"."Payment Method Code");
					 if RecFP.Reembolso then begin
						  if "Sales Invoice Header"."Shipping Agent Code"='DHL' then begin
							   Rec112.Reset;
							   Rec112.SetRange(Rec112."No.","Sales Invoice Header"."No.");
							   if Rec112.FindFirst then begin
									CUEXP.EtiDHLDESDEFAC(Rec112);
							   end;
						  end;
						  if "Sales Invoice Header"."Shipping Agent Code"='SEUR' then begin
							   Rec112.Reset;
							   Rec112.SetRange(Rec112."No.","Sales Invoice Header"."No.");
							   if Rec112.FindFirst then begin
									CUEXP.EtiSEURDESDEFAC(Rec112);
							   end;
						  end;
					 end;
				end;
				...
				primero:=true;
				RecDDP.Reset;
				RecDDP.SetRange(RecDDP."Table Name",1);
				RecDDP.SetRange(RecDDP.Code,Cust."Payment Days Code");
				if RecDDP.FindFirst then repeat
				   ///// falta  textodiaspago:=textodiaspago+' '+FORMAT(RecDDP."Payment Day")
				until RecDDP.Next=0;
				if RecClie."Centro ECI"<>'' then begin
				///// 5448
					 RecClie2.Get('5448');
					 if "Sales Invoice Header"."Bill-to Country/Region Code"='PT' then begin
						  RecClie2.Get('5542');
					 end;
					 ...
					 if "Sales Invoice Header"."Your Reference"='' then begin
						  Rec113.Reset;
						  Rec113.SetRange(Rec113."Document No.","Sales Invoice Header"."No.");
						  if Rec113.FindFirst then repeat
							   if Rec113."Shipment No."<>'' then begin
									if Rec110.Get(Rec113."Shipment No.") then begin
										 "Sales Invoice Header"."Your Reference":=Rec110."Your Reference";
									end;
							   end;
						  until Rec113.Next=0;
					 end;
	...
	var
		...
		RecDDP: Record "Payment Day";
		textodiaspago: Text[30];
		primero: Boolean;
		centro: Code[10];
	...
	begin
		...
		if NumVtoExtra > 0 then
		  ...
		  TextoExtra := '';
		Rec21.Reset;
		Rec21.SetCurrentkey("Document No.");
		Rec21.SetRange(Rec21."Document No.","Sales Invoice Header"."No.");
		Rec21.SetRange(Rec21."Document Type",21);
		Rec21.SetRange(Rec21."Source Code",'VENTAS');
		if Rec21.Find('-') then repeat
			Rec21.CalcFields(Rec21."Original Amount");
			if NumVto > MaxVto then begin
			  ImporExtra := ImporExtra + Rec21."Original Amount";
			  NumVtoExtra := NumVtoExtra + 1;
		///	END ELSE BEGIN
		///	  FechaVto[NumVto] := Rec21."Due Date";
		///	  ImpVto[NumVto] := Rec21."Original Amount";
		///	  NumVto := NumVto + 1;
			end;
		until Rec21.Next = 0
	end;
	...
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Cust."Impresion fact. sin dto."=false) and (DETALLEPVP=false));
		end;
	end;
	local procedure OnPreSectionTable113_Header8(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Cust."Impresion fact. sin dto."=false) and (DETALLEPVP=true));
		end;
	end;
	local procedure OnPreSectionTable113_Header9(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Cust."Impresion fact. sin dto."=true) and (DETALLEPVP=false));
		end;
	end;
	...
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Type > 0) and (Cust."Impresion fact. sin dto."=false) and (DETALLEPVP=false));
		end;
	end;
	local procedure OnPreSectionTable113_Body14(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Type > 0) and (Cust."Impresion fact. sin dto."=false) and (DETALLEPVP=true));
		end;
	end;
	local procedure OnPreSectionTable113_Body15(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput((Type > 0) and (Cust."Impresion fact. sin dto."=true) and (DETALLEPVP=false));
		end;
	end;
	...
	local procedure OnPreSectionTable113_Footer22(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
	...
	local procedure OnPreSectionTable113_Footer23(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
	...
	local procedure OnPreSectionTable113_Footer24(var "Sales Invoice Line" : Record "Sales Invoice Line");
	begin
		with Table113 do begin
		  ReportForNav.ShowOutput("Sales Invoice Header"."Prices Including VAT" and
		  (VATAmountLine."VAT Calculation Type" = VATAmountLine."vat calculation type"::"Reverse Charge VAT"));
	...
	var 
		[WithEvents]
		ReportForNav : DotNet ForNavReport50017_v7_3_0_2521;
	...
	var
		...
		path: DotNet System_IO_Path50017;
