...
Report 50905 "OK Albaran NO Valorado ECI"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/OK Albaran NO Valorado ECI.rdlc';
    Caption = 'OK Albaran Valorado ECI';
        ...
        dataitem("Sales Shipment Header";"Sales Shipment Header")
        {
            ...
            dataitem(CopyLoop;"Integer")
            {
                ...
                dataitem(PageLoop;"Integer")
                {
                    ...
                    column(numprovee;Customer."Nº proveedor")
                    {
                    }
                    column(surefeci;"Sales Shipment Header"."Your Reference")
                    {
                    }
                    column(TBULTOS;TBULTOS)
                    {
                    }
                    ...
                    dataitem("Sales Shipment Line";"Sales Shipment Line")
                    {
                        ...
                        column(VAT_SalesShipmentLine;"Sales Shipment Line"."VAT %")
                        {
                        }
                        column(Dtocabeceraoferta_SalesShipmentLine;"Sales Shipment Line"."Dto% cabecera/oferta")
                        {
                        }
                        column(LineDiscount_SalesShipmentLine;"Sales Shipment Line"."Line Discount %")
                        {
                        }
                        column(UnitCostLCY_SalesShipmentLine;"Sales Shipment Line"."Unit Cost (LCY)")
                        {
                        }
                        column(neto_SalesShipmentLine;neto)
                        {
                        }
                        column(total_SalesShipmentHdr;"Sales Shipment Header"."Total bultos")
                        {
                        }
                        ...
                        begin
                            ...
                            if "Sales Shipment Line".Type=2 then begin
                            ...
                            neto:=0;
                            if "Sales Shipment Line".Quantity<>0 then begin
                                 neto:="Sales Shipment Line"."Item Charge Base Amount"/"Sales Shipment Line".Quantity;
                            end;
                            ...
                            sumacantidad:=0;
                            Cajasporenvio.Reset;
                            Cajasporenvio.SetRange(Cajasporenvio.Nfac,"Sales Shipment Header"."No.");
                            Cajasporenvio.SetRange(Cajasporenvio.Producto,"Sales Shipment Line"."No.");
                            if Cajasporenvio.FindFirst then repeat
                              sumacantidad:=sumacantidad+Cajasporenvio.Cantidad;
                            until Cajasporenvio.Next=0;
                            ////// NUMETIQ:=1;

                            nueva1:=CopyStr(codssc,1,4)+CopyStr(codssc,6,4)+CopyStr(codssc,11,4)+CopyStr(codssc,16,4)+CopyStr(codssc,21,4);
                            ///// MESSAGE('%1 %2 %3',NUMETIQ,NUMETIQ,codssc);




                            "Sales Shipment Header".CalcFields("Grupo clientes");



                            RelacionproductogrupoMetros.Reset;
                            RelacionproductogrupoMetros.SetRange(RelacionproductogrupoMetros.Producto,"Sales Shipment Line"."No.");
                            RelacionproductogrupoMetros.SetRange(RelacionproductogrupoMetros."Grupo Cliente","Sales Shipment Header"."Grupo clientes");
                            if RelacionproductogrupoMetros.FindSet then begin
                                "Sales Shipment Line".Quantity:="Sales Shipment Line".Quantity*RelacionproductogrupoMetros.Metros;
                                "Sales Shipment Line"."Unit Price":=ROUND("Sales Shipment Line"."Unit Price"/RelacionproductogrupoMetros.Metros,0.01);
                                "Sales Shipment Line"."Precio base":=ROUND("Sales Shipment Line"."Precio base"/RelacionproductogrupoMetros.Metros,0.01);
                                "Sales Shipment Line"."Precio Tarifa":=ROUND("Sales Shipment Line"."Precio Tarifa"/RelacionproductogrupoMetros.Metros,0.01);
            ...
            begin
                ...
                if SalesShipmentLine.FindFirst then repeat
                     if (SalesShipmentLine.Type=SalesShipmentLine.Type::Item) and
                        (SalesShipmentLine.Quantity=0)  then begin
                ...
                if Customer.Get("Sales Shipment Header"."Sell-to Customer No.") then begin
                     centro:=Customer."Centro ECI";
                end;

                if Customer."Centro ECI"<>'' then begin
                ///// 5448
                     Customer2.Get('5448');
                     ...
                     ///FormatAddr.Customer(CustAddrdf,Customer2);
                end;
                dpto:='0219';
                ...
                NALBA:=CopyStr("Sales Shipment Header"."No.",1,2)+CopyStr("Sales Shipment Header"."No.",3,5);
    ...
    var
        ...
        NUMETIQ2: Integer;
        Cajasporenvio: Record UnknownRecord50025;
        RelacionproductogrupoMetros: Record UnknownRecord50040;
