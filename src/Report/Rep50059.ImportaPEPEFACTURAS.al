...
Report 50059 "_Importa PEPE FACTURAS"
{
    // YesNPEDIDO00
    // Yesfpago00
    // YesC300
    ...
    begin
        ...
        if RecCV.FindFirst then repeat
             if (RecCV.Ship=false) and (RecCV."Last Shipping No."='') then begin
                  if RecCV."Total peso"<>0 then begin
                       IMP21:=0;
                       IMP10:=0;
                       RecLV.Reset;
                       ...
                       if RecLV.FindFirst then repeat
                            ...
                            if RecLV."VAT Prod. Posting Group"='VAT21' then begin
                                  IMP21:=IMP21+RecLV.Quantity*RecLV."Unit Price";
                            end;
                            if RecLV."VAT Prod. Posting Group"='VAT10' then begin
                                 IMP10:=IMP10+RecLV.Quantity*RecLV."Unit Price";
                            end;
                       until RecLV.Next=0;
                       if (IMP10+IMP21<>0) then begin
                                 TOTAL1:=IMP10+IMP21;
                                 if IMP10<>0 then begin
                                      TANTO10:=ROUND(IMP10*100/TOTAL1,0.01);
                                      DTO2:=RecCV."Total peso";
                                      DTO2:=ROUND(DTO2*TANTO10/100,0.01);
                                      DTO2:=(DTO2/1.1);
                                      RecLV.Init;
                                      PRIMERAVEZ:=false;
                                      lineas:=lineas+10000;
                                      ...
                                      RecLV.Validate(RecLV."Unit Price",DTO2*-1);
                                      RecLV.Insert(true);
                                 end;
                                 if IMP21<>0 then begin
                                      TANTO21:=ROUND(IMP21*100/TOTAL1,0.01);
                                      DTO2:=RecCV."Total peso";
                                      DTO2:=ROUND(DTO2*TANTO21/100,0.01);
                                      DTO2:=(DTO2/1.21);
                                      RecLV.Init;
                                      PRIMERAVEZ:=false;
                                      lineas:=lineas+10000;
        ...
        if RecCV.FindFirst then repeat
             if (RecCV.Ship=false) and (RecCV."Last Shipping No."='') then begin
                  ...
                  if RecCV."imp. pdte"<>RecCV."Importe a cuenta" then begin
                       ...
                       lineas:=9999999;
                       if not RecLV.Get(1,RecCV."No.",lineas) then begin
                            RecLV.Init;
                            RecLV."Document Type":=1;
                            RecLV."Document No.":=RecCV."No.";
                            RecLV."Line No.":=lineas;
        ...
        ///                    RecLV.INSERT(TRUE);
                                 if IMP10<>0 then begin
                                      TANTO10:=ROUND(IMP10*100/DIFPED,0.01);
                                      DTO2:=RecCV."Total peso";
                                      DTO2:=ROUND(DTO2*TANTO10/100,0.01);
                                      DTO2:=(DTO2/1.1);
                                      RecLV.Init;
                                 ...
                                 if IMP21<>0 then begin
                                      TANTO21:=ROUND(IMP21*100/DIFPED,0.01);
                                      DTO2:=RecCV."Total peso";
                                      DTO2:=ROUND(DTO2*TANTO21/100,0.01);
                                      DTO2:=(DTO2/1.21);
                                      RecLV.Init;
    ...
    begin
        ...
        ///     TempDim.SETRANGE("Code Caption",UPPERCASE(FormatData(ExcelBuf."Cell Value as Text")));
             ...
             if (ExcelBuf."Column No."=1) then begin NPEDIDO:=ExcelBuf."Cell Value as Text";
                  pos:=0;
                  sale:=false;
                  REFPROD:='';
                  REFPROD2:=NPEDIDO;
                  repeat
                       pos:=pos+1;
                       if CopyStr(NPEDIDO,pos,1)<>'.' then begin
                            REFPROD:=REFPROD+CopyStr(NPEDIDO,pos,1);
                       end;
             ...
             clie:=ExcelBuf."Cell Value as Text";
                  ...
                  repeat
                       pos:=pos+1;
                       if CopyStr(clie,pos,1)<>'.' then begin
                            REFPROD:=REFPROD+CopyStr(clie,pos,1);
                       end;
                  until sale or (pos>StrLen(REFPROD2));
                  clie:=REFPROD;
             end;
    ...
    begin
        ...
        if not RecCV.Get(1,NPEDIDO) then begin
             Rec36Temp."Document Type":=1;
        ...
        CODCLIE:='MN'+clie;
        if Rec36Temp.Get(1,NPEDIDO) then begin
             if RecClie.Get(CODCLIE) then begin
                  Evaluate(ref,CREF);
                  if ref<>0 then begin
                       codprod:='';
                  ...
                  if RecProd.Get(codprod) then begin
                       BLOQE:=false;
                       if RecProd.Blocked then begin
                            RecProd.Blocked:=false;
                            RecProd.Modify;
                            BLOQE:=true;
                       end;
                       lineas:=lineas+10;
