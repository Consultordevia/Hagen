...
XmlPort 50085 ClasComerProd
{
    Caption = 'ClasComerProd';
    Direction = Import;
    FieldSeparator = ';';
    Format = VariableText;
    TextEncoding = UTF16;
    ...
    begin
        ...
        if RecItem.Get(codigo) then begin
            if UpperCase(CopyStr(Clasi, 1, 1)) = 'R' THEN BEGIN
                RecItem."Clasificación Comercial" := RecItem."Clasificación Comercial"::Rojo;
            END;
            if UpperCase(CopyStr(Clasi, 1, 1)) = 'A' THEN BEGIN
                RecItem."Clasificación Comercial" := RecItem."Clasificación Comercial"::Azul;
            END;
            if UpperCase(CopyStr(Clasi, 1, 1)) = 'V' THEN BEGIN
                RecItem."Clasificación Comercial" := RecItem."Clasificación Comercial"::Verde;
            END;
