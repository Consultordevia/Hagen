...
XmlPort 50017 dua
{
    ...
    begin
        ...
        if fecha <> '' then begin
            ...
            CDD := CopyStr(Format(fecha), 1, 2);
            CMM := CopyStr(Format(fecha), 4, 2);
            CAA := CopyStr(Format(fecha), 7, 4);
            Evaluate(dd, CDD);
            Evaluate(mm, CMM);
            Evaluate(aa, CAA);

            FECHALAZ := Dmy2date(dd, mm, aa);

        end;

        if PurchInvHeader.Get(codigo) then begin
            PurchInvHeader."Código importación" := codex;
            PurchInvHeader.DUA := ddua;
            PurchInvHeader."Fecha DUA" := FECHALAZ;
