...
XmlPort 50039 "Importa textos adi. PT"
{
    Caption = 'Importa textos adi. PT';
    Direction = Import;
    FieldSeparator = ';';
    Format = VariableText;
    TextEncoding =  UTF16;
    ...
    begin
        ...
        RecLT.Reset;
        RecLT.SetRange(RecLT."Table Name", 4);
        RecLT.SetRange(RecLT."No.", CODPROD);
        if RecLT.FindFirst then
            repeat
                RecLT.Delete;
            until RecLT.Next = 0;
        ...
        if not RecCT.Get(4, CODPROD, 'PTG', 1) then begin
            RecCT."Table Name" := 4;
            RecCT."No." := CODPROD;
            RecCT."Text No." := 1;
            RecCT."Language Code" := 'PTG';
            RecCT.Insert;
        end;
        if RecTP.Get(CODPROD, '', 'PTG') then begin
            RecTP.Description := CopyStr(texto, 1, 44);
            RecTP.Modify(true);
        end;
        if not RecTP.Get(CODPROD, '', 'PTG') then begin
            RecTP."Item No." := CODPROD;
            RecTP."Variant Code" := '';
            RecTP."Language Code" := 'PTG';
            RecTP.Description := CopyStr(texto, 1, 44);
            RecTP.Insert(true);
        end;
    end;
