...
XmlPort 50073 "Importacion SENDING2"
{
    Caption = 'Importacion SENDING2';
    Direction = Import;
    FieldSeparator = '|';
    Format = VariableText;
    TextEncoding =  UTF16;
    ...
    begin
        ...
        linea:=linea+1;
            ...
            if D14<>'' then begin
                ...
                repeat
                    x:=x+1;
                    if CopyStr(D14,x,1)='/' then begin
                        SALE:=true;
                    end;
                    if CopyStr(D14,x,1)<>'/' then begin
                        DDA:=DDA+CopyStr(D14,x,1);
                    end;
                until (x>=StrLen(D14)) or SALE;
                MMA:='';
                SALE:=false;
                repeat
                    x:=x+1;
