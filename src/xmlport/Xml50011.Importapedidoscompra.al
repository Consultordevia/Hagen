...
XmlPort 50011 "Importa pedidos compra"
{
    Caption = 'Alta Stock';
    Direction = Import;
    FieldSeparator = ';';
    ...
    begin
        ...
        if (D1 <> '') then begin
            ...
            ref := codprod;
            if not RecProd.Get(ref) then begin
                RecItem.Reset;
                RecItem.SetCurrentkey("Vendor Item No.");
                RecItem.SetRange(RecItem."Vendor Item No.", ref);
    ...
    procedure npedido(codpedido: Code[10]; ESOOP: Code[1])
    begin

        numpedi := codpedido;

        if ESOOP = '0' then begin
            TYPO := 0;
        end;
        if ESOOP = '1' then begin
            TYPO := 1;
        end;
    end;
}
