...
XmlPort 50509 "Importa datos nav2018-8cp"
{
    Caption = 'Importa datos nav2018-8cp';
    Direction = Import;
    FieldSeparator = ';';
    Format = VariableText;
    TextEncoding = UTF16;
    ...
    begin
        ...
        RECCP.RESET;
        RecCP.SetRange(code, d1);
        RecCP.SetRange(city, d2);
        if not RecCP.FindFirst() then begin
            v.Update(1, d1);
            v.Close();
            RecCP.Code := D1;
            RecCP.City := D2;
            RecCP."Search City" := D3;
            RecCP."Country/Region Code" := D4;
            RecCP.County := D5;
            RecCP."County Code" := D6;
            RecCP."Zona Venta" := D7;
            RecCP.Pais := D8;
            RecCP."Provincia shopyfi" := D9;
            RecCP."Código provincia shopyfi" := D10;
            RecCP."Provincia shopyfi CALC" := D11;
            RECCP.INSERT;
        END;
