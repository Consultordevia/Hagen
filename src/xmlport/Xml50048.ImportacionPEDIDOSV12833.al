...
XmlPort 50048 "Importacion PEDIDOS V-12833"
{
    ...
    begin
        ...
        if linea > 0 then begin
            if linea = 1 then begin
                clie := '';
                RecClie.Reset;
                RecClie.SetRange("No.", '12833');
                if RecClie.FindFirst then begin
                    clie := RecClie."No.";
                end;
                codacti := IncStr(RecUser."Serie pedidos");
                codacti := NoSeriesManagement.GetNextNo('V-PRESU-3', Today, true);
            ...
            if D13 <> '' then begin
                if RecProd.Get(D13) then begin
                    ...
                    if RecProd."Unidades venta" = 0 then begin
                        RecProd."Unidades venta" := 1;
                    end;
                    ...
                    if not SALE then begin
                        ...
                        RecLV.Type := 2;
                        RecLV.Validate(RecLV."No.", D13);
                        Evaluate(CANTIDE, D7);
                        if RecProd."Permite fraccionar venta" = false then begin
                            if RecProd."Unidades venta" >= CANTIDE then begin
                                CANTIDE := RecProd."Unidades venta";
                            end;
                        end;
                        if RecProd."Unidades venta" < CANTIDE then begin
                            multiplo := ROUND(CANTIDE / RecProd."Unidades venta", 1);
                            CANTIDE := multiplo * RecProd."Unidades venta";
                        end;
