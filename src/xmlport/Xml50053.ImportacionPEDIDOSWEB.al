...
XmlPort 50053 "Importacion PEDIDOS WEB"
{
    ...
    begin
        ...
        linea := linea + 1;
        Message('LINEA %1 %2 %3', D1, D20, linea);


        Message('1');
        if npedi <> D1 then begin
            Message('2');
            npedi := D1;
            if D20 <> '' then begin
                Message('3');
                clie := '';
                RecClie.Reset;
                RecClie.SetRange("E-Mail", D20);
                if RecClie.FindFirst then begin
                    clie := RecClie."No.";
                    if RecClie.Blocked <> RecClie.Blocked::" " then begin
                        currXMLport.Skip;
                        /*RecClie.Blocked:=0;
                        RecClie.MODIFY;
                        RecClieTemp."No.":=RecClie."No.";
                        RecClieTemp.INSERT;*/
                    end;
                end;
            ...
            codacti := NoSeriesManagement.GetNextNo('V-PED-WEB', Today, true);
            RecCV.Init;
    ...
    local procedure AltaCliente()
    begin



        clie := NoSeriesManagement.GetNextNo('CLIE', Today, true);
        RecCust."No." := clie;
        RecCust.Name := D18;
        RecCust."Search Name" := D18;
        RecCust."VAT Registration No." := D23;
        RecCust."E-Mail" := D20;
        RecCust.Address := CopyStr(D24 + D25, 1, 50);
        RecCust."Address 2" := CopyStr(D24 + D25, 51);
        RecCust."Post Code" := CopyStr(D26, 1, 10);
        RecCust.City := D27;
        RecCust."Phone No." := D28;
        RecCust."Nº movil" := D28;
        RecCust."Customer Posting Group" := 'NAC';
        RecCust."Customer Price Group" := CompanyInformation."Tarifa WEB";
        RecCust."Customer Disc. Group" := clie;
