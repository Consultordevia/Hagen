...
XmlPort 50051 "Importacion PEDIDOS tienda an2"
{
    // 
    // 
        ...
        if linea > 1 then begin
            if D2 <> codpedido then begin
                ...
                if not RecCV.FindFirst then begin
                    ...
                    codacti := NoSeriesManagement.GetNextNo('V-PED-TA', Today, true);
                    ...
                    RecCV.Validate(RecCV."Ship-to Post Code", D41);
                    RecCV.Validate(RecCV."Ship-to City", D42);
                    RecCV."Envio a-Nº Telefono" := D45;
                    RecCV."Observación para transporte" := D47;
