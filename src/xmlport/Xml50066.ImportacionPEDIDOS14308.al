...
XmlPort 50066 "Importacion PEDIDOS 14308"
{
    // 
    // 
    // Numero de pedido
        ...
        if linea > 2 then begin
            if linea >= 2 then begin
                ...
                if NPEDIDO <> D1 then begin
                    ...
                    RecCV."Ship-to Name" := CopyStr(D8, 1, 50);
                    RecCV."Ship-to Name 2" := CopyStr(D8, 51, 50);
                    RecCV."Ship-to Address" := CopyStr(D9, 1, 50);
                    RecCV."Ship-to Address 2" := CopyStr(D9, 51, 50);
                    RecCV."Ship-to Contact" := '';
                    RecCV.Validate(RecCV."Ship-to Post Code", D11);
                    RecCV.Validate(RecCV."Ship-to City", D10);
                    RecCV."Envio a-Nº Telefono" := D12;
                    X := 0;
                    SALE := false;
                    FECHAENVIO := Today;
