    ...
    begin
        ...
        /*
          X:=0;
                       ...
                       REPEAT
                            X:=X+1;
                            IF COPYSTR(fecha,X,1)='/' THEN BEGIN
                                 SALE:=TRUE;
                            END;
                            IF COPYSTR(fecha,X,1)<>'/' THEN BEGIN
                                 DDA:=DDA+COPYSTR(fecha,X,1);
                            END;
                       UNTIL (X=STRLEN(fecha)) OR SALE;
                       MMA:='';
                       SALE:=FALSE;
        
                       REPEAT
                            X:=X+1;
                            IF COPYSTR(fecha,1)='/' THEN BEGIN
                                 SALE:=TRUE;
                            END;
                            IF COPYSTR(fecha,X,1)<>'/' THEN BEGIN
                                 MMA:=MMA+COPYSTR(fecha,X,1);
                            END;
                       UNTIL (X=STRLEN(fecha)) OR SALE;
                                     AAA:=COPYSTR(fecha,X+1);
        ///               ERROR('%1 %2 %3 %4 %5',D[10],X,DDA,MMA,AAA);
        */

        DDA := CopyStr(fecha, 1, 2);
        MMA := CopyStr(fecha, 4, 2);
        AAA := CopyStr(fecha, 7, 2);
        Evaluate(DD, DDA);
        Evaluate(MM, MMA);
        Evaluate(AA, AAA);
        AA := AA + 2000;
        ...
        if DetailedCustLedgEntry.Get(num) then begin
            DetailedCustLedgEntry."Initial Entry Due Date" := FECHAENVIO;
