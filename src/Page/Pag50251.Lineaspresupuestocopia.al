...
Page 50251 "Lineas presupuesto copia"
{
    AutoSplitKey = true;
    ...
    begin
        ...
        ESMODIFICABLE := true;
        ESVISIBLE := true;
        //// ALBA EJEMPLO NI PUEDE VER NI EDITAR ///
        if UpperCase(UserId) = 'HAGEN\ALBA' then begin
            ESMODIFICABLE := false;
            ESVISIBLE := false;
        end;

        /// COMERCIALES NO PUEDEN MODIFICAR DESCUENTO LINEA-PRECIO UNITARIO-IMPORTE - Presupuesto ///

        if UpperCase(UserId) = 'HAGEN\COMERCIAL7' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL6' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL4' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL1' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL10' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL8' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\DALBANO' then begin
            ESMODIFICABLE := false;
        end;

        if UpperCase(UserId) = 'HAGEN\COMERCIAL14' then begin
            ESMODIFICABLE := false;
        end;
    end;
